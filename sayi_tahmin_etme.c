...
/*int sayiyiduzelt(int pcsayi,int sayaceksi,int sayacarti){
      int binlerbasamagi,yuzlerbasamagi,onlarbasamagi,birlerbasamagi;  // burada tekrardan bilgisayarin olusturdugu sayiyi basamaklara ayirdik.
      ...
      int tahmin[] = {birlerbasamagi,onlarbasamagi,yuzlerbasamagi,binlerbasamagi}; // Pc'nin tuttugu rakamlari diziye atadım.

      int i;
      for (i=0;i < 3;i++){

        birlerbasamagi = tahmin[i+1];
        onlarbasamagi = tahmin[i+2];
        yuzlerbasamagi = tahmin[i+3];
        binlerbasamagi = tahmin[i];

        pcsayi = birlerbasamagi+(onlarbasamagi*10)+(yuzlerbasamagi*100)+(binlerbasamagi*1000);
      }
...
int main()
{
  ...
  //    printf("toplam4:%d\n",toplam4);
      ...
      //sayiyiduzelt(pcsayi,sayaceksi2,sayacarti2);
